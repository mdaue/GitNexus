...
> Index stale? Run `node .gitnexus/run.cjs analyze` from project root (add `--embeddings` for semantic search, `--pdg` for taint/flow analysis). No `.gitnexus/run.cjs` yet? Bootstrap with `npx`, `bunx`, or `pnpm dlx` — e.g. `bunx gitnexus@latest analyze` (npm 11 npx crash; #1939).
